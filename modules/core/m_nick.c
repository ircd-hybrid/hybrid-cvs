 ...
 *
 *   $Id: m_nick.c,v 1.58 2001/03/06 02:22:29 androsyn Exp $
 */
#include "handlers.h"
#include "client.h"
#include "fdlist.h"
                  ...
                  int parc, char *parv[])
{
  ...
  if ((aclient_p = find_client(nick, NULL)))
    {
      /*
      ** If aclient_p == source_p, then we have a client doing a nick
      ** change between *equivalent* nicknames as far as server
      ** is concerned (user is changing the case of his/her
      ** nickname or somesuch)
      */
      if (aclient_p == source_p)
	{
      ...
      */
      if (!newts || !aclient_p->tsinfo
      ...
      else
        {
	      ...
	      kill_client_ll_serv_butone(source_p, aclient_p,
					 "%s (%s <- %s)",
					 me.name,
					 aclient_p->from->name,
					 get_client_name(client_p, HIDE_IP));
