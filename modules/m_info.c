 ...
 *
 * $Id: m_info.c,v 1.27 2001/03/06 02:05:13 androsyn Exp $
 */
#include "tools.h"
#include "m_info.h"
#include "channel.h"
...
static void send_conf_options(struct Client *server_p);
static void send_birthdate_online_time(struct Client *server_p);
static void send_info_text(struct Client *server_p);
...
void send_info_text(struct Client *server_p);
void send_birthdate_online_time(struct Client *server_p);
void send_conf_options(struct Client *server_p);
...
static void m_info(struct Client *client_p, struct Client *server_p,
                  int parc, char *parv[])
{
  static time_t last_used=0L;

  if (hunt_server(client_p,server_p,":%s INFO :%s",1,parc,parv) == HUNTED_ISME)
  {
    ...
    send_info_text(server_p);
    send_birthdate_online_time(server_p);

    sendto_one(server_p, form_str(RPL_ENDOFINFO), me.name, parv[0]);
  } /* if (hunt_server(client_p,server_p,":%s INFO :%s",1,parc,parv) == HUNTED_ISME) */
} /* m_info() */
...
**  parv[1] = servername
*/
static void mo_info(struct Client *client_p, struct Client *server_p,
                   int parc, char *parv[])
...
**  parv[1] = servername
*/
static void ms_info(struct Client *client_p, struct Client *server_p,
                   int parc, char *parv[])

{
  if (hunt_server(client_p,server_p,":%s INFO :%s",1,parc,parv) == HUNTED_ISME)
    {
      if(IsOper(server_p))
	mo_info(client_p,server_p,parc,parv);
      else
	m_info(client_p,server_p,parc,parv);
    }
} /* ms_info() */
...
static void send_info_text(struct Client *server_p)
{
  ...
  while (*text)
    sendto_one(server_p, form_str(RPL_INFO), me.name, server_p->name, *text++);
  
  sendto_one(server_p, form_str(RPL_INFO), me.name, server_p->name, "");
}
