 ...
 *
 * $Id: dline_conf.c,v 7.28 2001/02/16 14:36:23 db Exp $
 */
  ...
  PATRICIA_WALK(dline->head, node) 
	...
	conftype = 'D';
      get_printable_conf(node->data, &name, &host, &pass, &user, &port,
			 &classname);
      sendto_one(sptr, form_str(RPL_STATSDLINE), me.name, sptr->name,
		 conftype, host, pass);	
    } PATRICIA_WALK_END;
}
 ...
 * inputs	- pointer to client to report to
 * output	- none
 * side effects - report all ip I lines to sptr
 */
  ...
  PATRICIA_WALK(iline->head, node) 
    {
      get_printable_conf(node->data, &name, &host, &pass, &user, &port,
			 &classname);
      sendto_one(sptr, form_str(RPL_STATSDLINE), me.name, sptr->name, 'I',
		 host, pass);	
    } PATRICIA_WALK_END;
}
