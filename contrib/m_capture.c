 ...
 *  USA
 *
 *  $Id: m_capture.c,v 1.15 2005/07/16 12:19:38 michael Exp $
 */
  ...
  else
  {
    ...
    *p++ = '\0';
    ...
    DLINK_FOREACH(ptr, local_client_list.head)
    {
      ...
      if ((source_p == target_p) || IsOper(target_p) ||
          IsCaptured(target_p))
	continue;

      if (match(nick, target_p->name) && match(host, target_p->host) &&
    ...
    sendto_realops_flags(UMODE_ALL, L_ALL,
                         "Bulk captured %s!%s@%s, %d local match(es)",
             ...
             int parc, char *parv[])
{
  ...
  else
  {
    ...
    *p++ = '\0';
    ...
    sendto_realops_flags(UMODE_ALL, L_ALL,
                         "Bulk uncaptured %s!%s@%s, %d local match(es)",
                         nick, user, host, matches);
  }
}
